...
class DotFillerView(
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        count = ((measuredWidth.toFloat() - dotSize)/(dotSize + dashSize)).toInt()
        offset = (measuredWidth.toFloat() - (count * (dotSize + dashSize) + dotSize))/2f
        dotRect.set(
            0f,
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        canvas?.run {
            ...
            for(i in 0 until count) {
                drawRect(if(i == 0) dotFirstRect else dotRect, dotPaint)
                translate(dotSize + dashSize, 0f)
            }
